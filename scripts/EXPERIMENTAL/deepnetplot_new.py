...
def print_usage_repAndRec():
    ...
    print 'A : set all pixels to value p'
...
class InteractiveRepRecPlotter:
    ...
    def __init__(self, learner, vmat, image_width=None, char_indice=0):
        ...
        self.paramvals = {} # cached parameter values
        self.vmat = vmat
    ...
    def getParameterValue(self, paramname):
        if paramname not in self.paramvals:
            self.paramvals[paramname] = self.learner.getParameterValue(paramname)
        return self.paramvals[paramname]

    def getFilters(self, paramname):
        filters = self.getParameterValue(paramname)
        if len(filters)== self.image_width*self.image_height:
            filters = transpose(filters)
        return filters

    def getFilter(self, paramname, i):
        filter = self.getParameterRow(paramname,i)
        if len(filter)!= self.image_width*self.image_height:
            filter = self.getParameterColumn(paramname,i)
        return filter

    def getParameterRow(self, paramname, i):
        M = self.getParameterValue(paramname)
        return M[i]

    def getParameterColumn(self, paramname, i):
        M = self.getParameterValue(paramname)
        return M[:,i]
    ...
    def __repCommands(self,event):
        ...
        if i != -1:
            ...
            if char == '!':
                ...
                print "-> Sampling type is now: ", self.sampling_type
            # set k -- k
            elif char == 'k':
            ...
            # plot W and M -- w
                ...
                if nameW in listNames and nameM in listNames:
                    ...
                    rowW = self.getFilter(nameW,n%hl.groupsize)
                    rowM = self.getFilter(nameM,int(n/hl.groupsize))
...
#                         draw()
            ...
            #like 'w' but for an entire row -- 'W'
                ...
                if nameW in listNames and nameM not in listNames:                
                    ...
                    print self.getFilters(nameW).shape
                   
                    figure(3)
                    myioff()                    
                    clf()
                    plotLayer1(self.getFilters(nameW), self.image_width, .1, n, hl.groupsize,.05, None, [], names, self.same_scale)
                    myion()
                    draw()
                ...
                if nameW in listNames and nameM in listNames:
                    ...
                    w = self.getFilters(nameW)
                    m = self.getFilters(nameM)
            ...
            # like 'w' on the max of each row -- 'C'
                ...
                if nameW in listNames and nameM not in listNames:
                    ...
                    myioff()                    
                    clf()
                    plotLayer1(self.getFilters(nameW), self.image_width, .056, 0,0,.05, self.from1568to784functions[self.from1568to784function], indexes,names, self.same_scale)
                    myion()
                    draw()
                ...
                if nameWr in listNames and nameMr not in listNames:
                    ...
                    myioff()
                    clf()
                    plotLayer1(self.getFilters(nameWr), self.image_width, .056,0,0,.05,self.from1568to784functions[self.from1568to784function],indexes,names, self.same_scale)
                    myion()
                    draw()
