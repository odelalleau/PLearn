    ...
    if program_path is None:
        ...
        if len(wlines) != 1:
            raise core.PyTestUsageError(
                ...
                "or those found in <plearn_branch>/commands/. Program %s was not found. "
                "Call 'which %s' resulted in those %d lines \n%s"
                %(self.name, len(wlines), ''.join(wlines)) )
...
class Program(core.PyTestObject):
    ...
    def __init__(self, **overrides):
        ...
        if self.compiler:
            ...
            self.log_file_path = internal_exec_path+'.log'
    ...
    def compile(self):        
        if self.compilationSucceeded():
            logging.debug("Already successfully compiled %s"%self._exec_name)
            return True

        elif self.__attempted_to_compile:
            logging.debug("Already attempted to compile %s"%self._exec_name)
            return False

        #######  First compilation attempt  ####################################
        
        targetdir, exec_name = os.path.split(self.getInternalExecPath())
        sanity_check, log_fname = os.path.split(self.log_file_path)
    ...
    def getProgramPath(self):
        ...
        try:
            ...
            try:
                ...
                logging.critical("%s %s"%(not_local, neither_global))
        return self._program_path
